class VacationsController < ApplicationController
  ...
  def check_vacation_days
    ...
    if fiscal_year_of_start_date != fiscal_year_of_end_date and @employee.vacations.where(vacation_type: 'Vacation', status: [nil, '']).reduce(0.0) { |sum, vacation| sum += vacation.pdo_taken(fiscal_year_of_end_date) } > @employee.max_vacation_days(Date.new(fiscal_year_of_end_date))
